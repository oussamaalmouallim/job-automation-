...
export default function PostGenerator() {
  ...
  const handleGenerate = async () => {
    if (!form.description.trim()) {
      toast.error('Decris ton projet ou ton sujet.')
      return
    }
    ...
    try {
      ...
      const text = res?.post || res?.text || res?.content ||
        res?.choices?.[0]?.message?.content ||
        (typeof res === 'string' ? res : JSON.stringify(res))

      setResult(text)
      addPost({ type: form.type, model: form.model, content: text })
      toast.success('Post genere !')
    } catch (err) {
      console.error(err)
  ...
  return (
    <div className="max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="card space-y-4">
        <h2 className="font-semibold text-gray-800">Parametres du post</h2>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Ex: J'ai automatise l'extraction de profils LinkedIn avec n8n..."
          />
        </div>
        ...
        <button className="btn-primary w-full" onClick={handleGenerate} disabled={loading}>
          {loading ? 'Generation...' : 'Generer le post'}
        </button>
      </div>

      <div className="card flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-gray-800">Post genere</h2>
          {result && (
            <span className="text-xs text-gray-400">{result.length} caracteres</span>
          )}
        </div>
        ...
        {result ? (
          <>
            ...
            <div className="flex gap-2">
              ...
              <button className="btn-secondary" onClick={handleGenerate}>Regenerer</button>
            </div>
          </>
        ) : (
              ...
              <div key={p.id} className="flex items-start justify-between gap-3 bg-gray-50 rounded-lg px-3 py-2">
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-gray-400">
                    {new Date(p.createdAt).toLocaleDateString('fr-FR')} - {p.type} - {p.model}
                  </p>
                  <p className="text-sm text-gray-700 truncate">{p.content}</p>
                </div>
                <button
                  className="btn-secondary text-xs shrink-0"
                  onClick={() => { setResult(p.content); toast.success('Post charge') }}
                >
                  Charger
                </button>
              </div>
